...
Shopify.contentCreator.gtm = {
  pushHomeBannerDatalayer: (nonInteraction, action, ecommerce, promotionId, promotionName, creative, position) => {
  ...
  pushProductDatalayer: (nonInteraction, action, ecommerce) => {
    window.dataLayer.push({
      "event": "eeEvent",
      "eventCategory": "EnhancedEcommerce",
      "eventAction": action,
...
Shopify.contentCreator.util = {
  isInViewPortOfTwo: function (el, distance = 0) {
    const viewPortHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight
    const top = el.getBoundingClientRect() && el.getBoundingClientRect().top;
    return top <= (viewPortHeight + distance);
  },
  imgSeturl: function () {
    var width = $(window).width();
    $('img[data-seturl]').each(function () {
      if (width > 769) {
  ...
  closeModal() {
    $('#util-modal').css("display", "none");
  },
  openModal(con) {
    $('#util-modal').css("display", "block");
    $('#util-modal #modal_con').html(`${con}`);
  },
  ...
  debounce: function (func, wait, options) {
    ...
    let lastThis; // 保存上一次 this
    let maxWait; // 最大等待时间，数据来源于 options，实现节流效果，保证大于一定时间后一定能执行
    let result; // 函数 func 执行后的返回值，多次触发但未满足执行 func 条件时，返回 result
    let timerId; // setTimeout 生成的定时器句柄
    let lastCallTime; // 上一次调用 debounce 的时间
    ...
    // 获取用户传入的配置 options
    if (Shopify.contentCreator.util.isObject(options)) {
    ...
    // ----------- 开闭定时器 -----------
    // 开启定时器
    function startTimer(pendingFunc, paramWait) {
      // 没传 paramWait 时调用 window.requestAnimationFrame()
      if (useRAF) {
      ...
      // 不使用 RAF 时开启定时器
      return setTimeout(pendingFunc, paramWait);
    }
  ...
  throttle: function (func, wait, options) {
    ...
    return Shopify.contentCreator.util.debounce(func, wait, {
      leading,
      trailing,
      maxWait: wait,
    });
...
Shopify.contentCreator.pastApi = {
  getQuery: () => {
  ...
  getUserInfo: ({ fn, error, params }) => {
    if (params.email) params.email = encodeURIComponent(params.email);
    const query = Shopify.contentCreator.util.getApiQuery(params);
    if (fn && typeof fn === "function") {
      $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: `/apps/pp/udc/users/private_profile${query}`,
        type: "GET",
  ...
  subscribe_emails: ({ fn, error, params }) => {
    if (fn && typeof fn === "function") {
      $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: "/apps/pp/udc/subscribe_emails",
        type: "POST",
        contentType: "application/json",
        dataType: "json",
        data: JSON.stringify({ ...Shopify.cookie.getAdCookie(), ...params }),
        success: (body) => {
  ...
  subscribe_deals_emails: ({ fn, error, params }) => {
    const { deals_type = "" } = params;
    if (!window.grecaptcha) return;
    if (fn && typeof fn === "function") {
      grecaptcha.ready(() => {
        grecaptcha
          .execute(ShopMetafields.recaptcha, { action: "template_subscribe" })
          .then((recaptcha) => {
            params["g-recaptcha-response"] = recaptcha;
            $.ajax({
              headers: Shopify.contentCreator.pastApi.getHeaders(),
              url: `/apps/pp/rainbow/deals/${deals_type}/subscribe`,
              type: "POST",
              contentType: "application/json",
              dataType: "json",
              data: JSON.stringify({
  ...
  subscribe_activities_emails: ({ fn, error, params }) => {
    if (!window.grecaptcha) return;
    if (fn && typeof fn === "function") {
      grecaptcha.ready(() => {
        grecaptcha
  ...
  activities_info_add: ({ fn, error, params }) => {
    if (!window.grecaptcha) return;
    if (fn && typeof fn === "function") {
      grecaptcha.ready(() => {
        grecaptcha
          ...
          .then((recaptcha) => {
            ...
            $.ajax({
              headers: Shopify.contentCreator.pastApi.getHeaders(),
              url: "/apps/pp/rainbow/deals/old_change_new_deals/subscribe",
              type: "POST",
              contentType: "application/json",
              dataType: "json",
              data: JSON.stringify({
  ...
  post_issue: ({ fn, error, params }) => {
    if (fn && typeof fn === "function") {
      $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: "/apps/pp/jeecg/jira/createIssuewithAttachment",
        type: "POST",
        contentType: "application/json",
        dataType: "json",
...
Shopify.contentCreator.friendbuy = {
  friendbuyv2_getAccountInfo: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    if (fn && typeof fn === "function") {
      return $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: `/apps/pp/friendbuyv2/getAccountInfo${query}`,
        type: 'GET',
        success: function (body) {
          fn(body);
        },
  ...
  getCustomer: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    if (fn && typeof fn === "function") {
      return $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v1/customer_info${query}`,
        type: 'GET',
        success: function (body) {
          fn(body);
        },
  ...
  getCustomerV2: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    if (fn && typeof fn === "function") {
      return $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/customers${query}`,
        type: 'GET',
        success: function (body) {
          fn(body);
        },
  ...
  getSharesList: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    return $.ajax({
      headers: Shopify.contentCreator.pastApi.getHeaders(),
      url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/shares${query}`,
      type: 'GET',
      success: function (body) {
        if (error && typeof fn === "function") fn(body);
      },
      error: (err) => {
        if (error && typeof error === "function") error(err);
      }
  ...
  getPurchasesList: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    return $.ajax({
      headers: Shopify.contentCreator.pastApi.getHeaders(),
      url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/purchases${query}`,
      type: 'GET',
      success: function (body) {
        if (error && typeof fn === "function") fn(body);
      },
  ...
  getConversionsList: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    return $.ajax({
      headers: Shopify.contentCreator.pastApi.getHeaders(),
      url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/conversions${query}`,
      type: 'GET',
      success: function (body) {
        if (error && typeof fn === "function") fn(body);
      },
  ...
  getRewardsList: ({ fn, error, params = {} }) => {
    const query = Shopify.contentCreator.util.getApiQuery(params);
    return $.ajax({
      headers: Shopify.contentCreator.pastApi.getHeaders(),
      url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/rewards${query}`,
      type: 'GET',
      success: function (body) {
        if (error && typeof fn === "function") fn(body);
      },
  ...
  postEmails: ({ fn, error, params = {} }) => {
    if (fn && typeof fn === "function") {
      return $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/shares`,
        type: 'POST',
        contentType: "application/json",
        dataType: "json",
        data: JSON.stringify({ ...params }),
  ...
  postEmails: ({ fn, error, params = {} }) => {
    if (fn && typeof fn === "function") {
      return $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: `/apps/pp/${Shopify.metafields.friendbuy.id}/friendbuy/v2/purchases`,
        type: 'POST',
        contentType: "application/json",
        dataType: "json",
        data: JSON.stringify({ ...params }),
...
Shopify.contentCreator.referral = {
  getCaptcha: ({ fn, error, params }) => {
    if (fn && typeof fn === "function") {
      $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
  ...
  referral_emails: ({ fn, error, params }) => {
    if (fn && typeof fn === "function") {
      $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: '/apps/pp/rainbow/deals/eufy_referral_email',
        type: 'POST',
        contentType: "application/json",
        dataType: "json",
        data: JSON.stringify({ ...Shopify.cookie.getAdCookie(), ...params }),
  ...
  googleContacts: ({ fn, error, params }) => {
    if (fn && typeof fn === "function") {
      $.ajax({
        headers: Shopify.contentCreator.pastApi.getHeaders(),
        url: '/apps/pp/google/m8/feeds/contacts/default/full',
        type: 'get',
        contentType: "application/json",
        dataType: "json",
        data: params,
        success: (body) => fn(body),
        error: (err) => {
          if (error && typeof error === "function") error(err);
        }
      });
...
Shopify.cookie = {
  getAdCookie: () => {
    ...
    const query = Shopify.cookie.getQuery();
    const refAds = Shopify.cookie.getUtm(query);
    return {
      fbuy_ref_code: query.fbuy_ref_code || Cookies.get('fbuy_ref_code'),
      affiliate: query.affiliate || Cookies.get('affiliate'),
      ref: refAds || Cookies.get('ref_ads'),
      ...
      deals_type: query.deal || Cookies.get('deal'),
      transfer_id: query.transfer_id || Cookies.get('transfer_id')
    };
  },
  addCookie: () => {
    const query = Shopify.cookie.getQuery();
    ...
    const domain = hostUrl[0].indexOf('beta-') > -1 ? window.location.host : `.${hostUrl[1]}.${hostUrl[2]}`;
    if (refAds) {
      Cookies.set('ref_ads', refAds, { domain: domain, expires: 14 });
      Cookies.set('reg_source', window.location.href, { domain: domain, expires: 14 });
      Cookies.set('ref_query', window.location.search || '', { domain: domain, expires: 14 });
    }
    // Google gclid参数，用于支付广告追踪
    if (query.gclid) Cookies.set('affiliate', `gclid=${query.gclid}`, { domain: domain, expires: 14 });
    if (query.tag) Cookies.set('pla_tag', query.tag, { domain: domain, expires: 14 });
    if (query.aic) Cookies.set('aic', query.aic, { domain: domain, expires: 14 });
    if (query.deal) Cookies.set('deal', query.deal, { domain: domain, expires: 14 });
    if (query.ic || query.inviter_code) {
      // 给其他API使用
      Cookies.set('inviter_code', query.ic || query.inviter_code, { domain: domain, expires: 14 });
    }
    // for postback(studentbeans)
    if (query.aff_sub) Cookies.set('aff_sub', query.aff_sub, { domain: domain, expires: 14 });
    // friendbuy refer code
    if (query.fbuy_ref_code) Cookies.set('fbuy_ref_code', query.fbuy_ref_code, { domain: domain, expires: 14 });
  },
  getUtm: (query) => query.utm_source && `utm_source=${query.utm_source || ''}&utm_media=${query.utm_media || query.utm_medium
    || ''}&utm_content=${query.utm_content || ''}&utm_campaign=${query.utm_campaign || ''}&utm_term=${query.utm_term || ''}`,
  getQuery: () => {
    const url = window.location.search; //获取url中"?"符后的字串
    const theRequest = new Object();
    if (url.indexOf("?") != -1) {
...
Shopify.contentCreator.activities = {
  get_sales_by_code: ({ params, onSuccess, onError }) => {
    $.ajax({
      type: 'GET',
      url: '/apps/pp/shopifyactivities/orders/sales_by_code',
      contentType: "application/json",
      dataType: "json",
      data: params,
      success: body => {
  ...
  openSuccessBox: (txt) => {
    openModal(`<div><img src='https://cdn.shopify.com/s/files/1/0582/4669/3040/files/tick.svg?v=1631256653'><br/><p>${txt}</p></div>`);
  }
};
...
Shopify.theme.addItemsToCart = function addItemsToCart(variants, { onSuccess, onError, onDone }) {
  jQuery.post('/cart/add.js', { items: variants })
    .done(data => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        priceAll += element.price
        productName.push(element.title)
        productType.push(element.type)
      }
      ...
      fbq('track', 'AddToCart',
        ...
        {
          value: priceAll / 100,
          num_items: quantityAll,
          currency: '{{ shop.metafields.global.currency }}',
          content_name: productName.length > 1 ? productName.join(',') : productName,// 产品内容的名字
          content_type: 'product_group',
          content_ids: variant_ids, // 产品的varaint id
          content_category: productType.length > 1 ? productType.join(',') : productType,  // 产品分类
        }
      ...
      variants.map(({ id }) => {
        ...
        const { handle, sku, title } = target
        Shopify.theme.jsAjaxCart.showDrawer(target);
    ...
    .fail(({ responseText }) => {
      let err = {};
      try { err = JSON.parse(responseText) } catch (e) { console.error(responseText); };
      if (onError) onError(err);
    })
    .always(() => { if (onDone) onDone(); });
};

Shopify.theme.bundleBuynow = function bundleBuynow({ authorization, checkout, onSuccess, onError }) {
  const checkoutdata = checkout
  let { discount } = checkout

  if (discount && typeof encodeURIComponent === 'function') {
  ...
  $.ajax({
    ...
    success: data => {
      if (data.checkout.line_items) {
        const arr = []
        for (let i = 0; i < data.checkout.line_items.length; i++) {
          const element = data.checkout.line_items[i];
          arr.push(element.variant_id)
        }
        fbq('track', 'InitiateCheckout',
          {
            value: data.checkout.total_price,
            num_items: 1,
      ...
      const { line_items = [] } = data.checkout;
      line_items.length && line_items.map(({ title, sku }) => referralTrack('products', title, 'click', { name: 'buyNow', sku }));
      if (onSuccess) onSuccess();
      window.location.href = discount ? (data.checkout.web_url.indexOf('?') > -1 ? `${data.checkout.web_url}&discount=${discount}` : `${data.checkout.web_url}?discount=${discount}`) : data.checkout.web_url;
    },
    error: ({ responseText }) => {
      let err = {};
      try { err = JSON.parse(responseText) } catch (e) { console.error(responseText); };
...
Shopify.theme.bundleBuynow_BF = function bundleBuynow_BF({ authorization, checkout, onSuccess, onError }) {
  const checkoutdata = checkout
  let { page_group, page_position, discount } = checkout;
  ...
  $.ajax({
    ...
    success: body => {
      ...
      window.dataLayer.push({
        ...
        'eventLabel': line_items.map(item => item.sku).join('_'),
        'ecommerce': {
          'currencyCode': checkout.currency, // 填写对应货币 USD EUR...
          'add': {
            'products': (function () {
              return line_items.map(function (item) {
                return {
                  'name': item.title, // 对应下方截图的product name
                  'id': item.product_id, // 对应下方截图的product id (注意大小写)
                  'category': '', // 留空
                  'categoryName': 'Black Friday', // 对应下方截图的product cat...
                  'brand': 'Eufy', // 对应下方截图的product brand
                  'variant': item.variant_title, // 对应下方截图的product variant
                  'price': item.price, // 对应下方截图的product price
                  'quantity': item.quantity // 对应下方截图的product quantity
                }
              })
            })()
          }
      ...
      window.location.href = discount ? (checkout.web_url.indexOf('?') > -1 ? `${checkout.web_url}&discount=${discount}` : `${checkout.web_url}?discount=${discount}`) : checkout.web_url;
      if (onSuccess) onSuccess();
    },
    error: ({ responseText }) => {
      let err = {};
...
Shopify.theme.mobileMenu = {
  ...
  open: function open() {
    ...
    $('.mobile-header').addClass('is-hover')
  ...
  close: function close() {
    $('html, body').removeClass('mobile-menu--opened'); // Once mobile menu is closed, return back to previous position on page

    var lastScrollPosition = $('body').attr('data-current-position');
    window.scrollTo(0, +lastScrollPosition);
    this.$mobileMenuIcon.removeClass('is-active');
    $('.mobile-header').removeClass('is-hover')
...
function parseQueryString(url) {
  ...
  var key = "",
    value = "";
  var paraString = url && url.substring(url.indexOf("?") + 1, url.length).split("&");
...
function referralTrack(menu, sub, type, data, extraData = '') {
  const { securityUtm, eufyhomeUtm, eufylifeUtm } = Shopify.metafields.referrals || {};
  const { ref, inviter_code } = Shopify.cookie.getAdCookie();
  const ref_ads = parseQueryString(ref);
  const userAgent = window && window.navigator.userAgent;
  let pageRef = " ";
  if (location.href) {
    let host = location.href.indexOf('?') != -1 ? location.href.split('?')[0] : location.href;
    pageRef = host.indexOf('/pages/') != -1 ? host.split('/pages/')[1] : '';
  }
  const postdata = {
    "events": [
      {
        ab_code: (Country.code || "us").toUpperCase(),
        business: 'Referral',
        ...
        event_type: {
          page: 10001,
          click: 10100
        }[type], // 进入页面(或回到页面) 10002-页面加载完成 10003-退出页面 10100-点击事件
        extra1: ref + extraData, // 保留字段1
        extra2: `{"userAgent": "${userAgent}"}`, // 保留字段2
        log_version: 100, // 日志版本号，100-表示1.00版本
        module: menu, // 所属模块，如: ShopHomePage
        sub_module: sub, // 所属子模块，如: ReferalInviteeHomePage
        ...data && { click_data: { ...data, label: pageRef } }, // 点击数据
        ...inviter_code && { page_data: { message: inviter_code } }, // 页面数据
        session_id: Cookies.get('_ga'), // 会话ID(UserID后4位_时间戳)，如: f22a_1583251646199
        source: ref_ads.utm_term, // 来源，如: H5(来自H5) App(来自App) Web(来自Web端),
        user_type: 10000, // 用户类型，0-未定义 1-没有任何产品 2-被分享用户 3-owner用户 4-owner+被分享用户
      }
    ]
  }

  // cookie utm判断是否匹配security/eufyhome统计条件
  const utm =
    (ref_ads.utm_source === securityUtm.utm_source && ref_ads.utm_media === securityUtm.utm_media) && securityUtm
    || (ref_ads.utm_source === eufyhomeUtm.utm_source && ref_ads.utm_media === eufyhomeUtm.utm_media) && eufyhomeUtm
    || (ref_ads.utm_source === eufylifeUtm.utm_source) && eufylifeUtm;
  // 如果满足则触发统计事件
  if (utm) {
    $.ajax({
      url: `/apps/pp/${utm.target}`,
      type: 'POST',
...
Shopify.theme.pushdataLayer = {
  pushQuantityBox: function pushQuantityBox(cartItems, $el) {
    // datalayer
    window.dataLayerData.cartItems = cartItems;
    var $productContent = $el.parents('.ajax-cart__product');
    var $cartCard = $el.parents('.cart__card');
    var jsCartItem = {};
    cartItems.map(item => {
    ...
    var QuantityEcommerce = {
      ...
      "items": [{
        ...
        "item_brand": window.dataLayerData.item_brand,
    ...
    if ($productContent.length > 0) {
      if ($el.data('update-quantity') === 'plus') {
        window.dataLayer.push({
          "event": "add_to_cart",
          "page_group": window.dataLayerData.page_group,
...
Shopify.theme.quantityBox = {
  ...
  updateQuantity: function updateQuantity($el) {
    ...
    if ($el.parents('[data-line-item]').length) {
      ...
      Shopify.theme.quantityBox.updateCart(_lineID, val, $el);
    } // Call to update quantity controls


    Shopify.theme.quantityBox.updateQuantityControls($el);
  },
  updateCart: function updateCart(lineID, quantity, $el) {
    ...
    $.ajax({
      ...
      success: function success(cart) {
        Shopify.theme.pushdataLayer.pushQuantityBox(cart.items, $el);
        var newQuantity = 0;
        var itemsLeftText = '';
...
var selectCallback = function selectCallback(variant, selector) {
  ...
  if (variant && variant.available == true) {
    ...
    } else if (variant.price == 999999999) {
      $('.current_price', $product).html('<span class="product-thumbnail__sold-out sold-out">' + Shopify.translation.soldOut + '</span>');
      $('.purchase-details').addClass('swap--visible');
      $('.listing_product .product_swatch').css('paddingBottom', '0');

      if ($('.listing_product')) {
        // out-stock-notify
        $('.listing_product .out-stock-notify').removeClass('swap--visible');
        $('.listing_product .out-stock-notify').css('display', 'block');
        $('.out-stock-pop').css('display', 'block');
        $('.notify-btn').css('display', 'block');
        $('.purchase-details').removeClass('borderB');
      }
    } else {
      $('.current_price', $product).html('<span class="money">' + Shopify.formatMoney(variant.price, $('body').data('money-format')) + '</span><span class="price_txt">' + window.price_tax + '</span>');
      $('.purchase-details').removeClass('swap--visible');
      $('.listing_product .product_swatch').css('paddingBottom', '24px');

      if ($('.listing_product')) {
        // out-stock-notify
        $('.listing_product .out-stock-notify').css('display', 'none');
        $('.listing_product .out-stock-pop').css('display', 'none');
        $('.listing_product .notify-btn').css('display', 'none');
    ...
    if ($('.listing_product')) {
      ...
      $('.listing_product .out-stock-notify').removeClass('swap--visible');
      $('.listing_product .out-stock-notify').css('display', 'block');
      $('.listing_product .out-stock-pop').css('display', 'block');
      $('.listing_product .notify-btn').css('display', 'block');
    }
  }
};
...
Shopify.theme.getURLParams = function (name, url) {

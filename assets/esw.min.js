(function (l) {
  ...
  var m = [".salesforce.com", ".force.com", ".sfdc.net", ".site.com", ".salesforce-sites.com"],
    n = "liveagent.chatCanceledOnDifferentTab liveagent.fileTransfer.resetFileSelector liveagent.fileTransfer.uploadFile session.deletedSessionData session.onLoad session.sessionData session.updateMaster".split(" ");
  d.prototype.getLightningOutParamsObj =
    function () {
      var a = {};
  ...
  d.prototype.adjustCommunityStorageDomain = function () {
    this.isCommunityDomain(this.settings.storageDomain) &&
      this.settings.storageDomain === document.domain && (this.settings.storageDomain = this.settings.storageDomain + "/" + window.location.pathname.split("/")[1])
  };
  ...
  d.prototype.instantiateLightningOutApplication = function (a) {
    if ("function" !== typeof Promise) this.loadScriptFromDirectory("common", "promisepolyfill",
      ...
      try {
        ...
        embedded_svc.utils.isCommunityOrSite() ? b("Communities context already has an Aura context.") : window.$Lightning && $Lightning.use("embeddedService:sidebarApp",
          function () {
  ...
  d.prototype.createEmbeddedServiceComponent = function (a) {
    if ("function" !== typeof Promise) this.loadScriptFromDirectory("common", "promisepolyfill", function () {
      ...
      try {
        ...
        window.$Lightning && !document.querySelector(".embeddedServiceSidebar") ? $Lightning.ready($Lightning.createComponent.bind(this, "embeddedService:sidebar", f, h, function (g, k, p) {
  ...
  d.prototype.validateStartChatAttributes = function (a) {
    ...
    else throw Error("Validation failed for fallbackRouting, received: " +
      f);
  ...
  d.prototype.outputToConsole = function (a, b, c) {
    if ((c || this.settings.devMode) && console && console[a]) console[a]("[Snap-ins] " +
  ...
  d.prototype.deprecated = function (a) {
    this.warning(a + " is deprecated in version " +
      Number("5.0").toFixed(1) + " and will be removed in version " + (Number("5.0") + 1).toFixed(1))
  };
  ...
  d.prototype.mergeSettings = function (a) {
    Object.keys(a).forEach(function (b) {
      void 0 ===
        this.settings[b] && (this.settings[b] = a[b])
    }.bind(this))
  };
  ...
  d.prototype.getESWFrame = function () {
    if (!this.eswFrame) {
      ...
      a && a.contentWindow && (this.eswFrame = a.contentWindow)
    }
  ...
  d.prototype.loadCSS = function () {
    ...
    a.href = (this.settings.gslbBaseURL ? this.settings.gslbBaseURL : this.settings.baseCoreURL) +
      "/embeddedservice/" + this.settings.releaseVersion + "/esw" + (this.settings.devMode ? "" : ".min") + ".css";
    a.type = "text/css";
    a.rel = "stylesheet";
  ...
  d.prototype.addSessionHandlers = function () {
    ...
    this.addMessageHandler("session.frameReady", function (a) {
    ...
    if (!this.componentInitInProgress && !document.getElementsByClassName("embeddedServiceSidebar").length) {
      ...
      try {
        embedded_svc.utils.isCommunityOrSite() &&
          0 < Object.keys(embedded_svc.liveAgentAPI.inviteButton).length ? window.dispatchEvent(new CustomEvent("onStartChatFromInvitation"), {
            detail: {
              bubbles: !0,
              composed: !1
            }
  ...
  d.prototype.resumeInitWithSessionData = function (a) {
    var b = embedded_svc.utils.fireEvent("sessionDataRetrieved", function (f) {
      ...
      c = !1,
      e = !1;
    this.settings.linkAction.valid ? c = !0 : b ? (this.log("Existing session found. Continuing with data: " +
      a), e = c = !0, embedded_svc.menu && embedded_svc.menu.hideTopContainer()) : this.componentInitInProgress && (c = !0);
  ...
  d.prototype.checkAuthentication = function () {
    this.isAuthenticationRequired && !this.settings.allowGuestUsers ? this.auth.oauthToken ?
      (this.loginButtonPressed || this.componentInitInProgress) && this.initLightningOut(this.loginPendingSerializedData) : embedded_svc.utils.fireEvent("requireauth") : (this.loginButtonPressed || this.componentInitInProgress) && this.initLightningOut(this.loginPendingSerializedData)
  };
  ...
  d.prototype.loadScriptFromDirectory = function (a, b, c, e) {
    ...
    f.id = g;
    f.type = "text/javascript";
    f.src = [h, "embeddedservice", e ? void 0 : this.settings.releaseVersion, a, b + (this.settings.devMode ? "" : ".min") + ".js"].filter(function (k) {
      return !!k
    }).join("/");
    c && (f.onload = c);
  ...
  d.prototype.addEventHandler = function (a, b) {
    window.embedded_svc &&
      embedded_svc.utils ? embedded_svc.utils.addEventHandler(a, b) : (this.storedEventHandlers[a] || (this.storedEventHandlers[a] = []), this.storedEventHandlers[a].push(b))
  };
  d.prototype.setupMessageListener = function () {
    window.addEventListener("message", function (a) {
      var b = a.data,
        c = a.origin !== "null" ? a.origin.split(":")[1].replace("//", "") : "",
        e = embedded_svc.utils ? embedded_svc.utils.isProtocolHttpOrHttps(a.origin) : !1,
        f = embedded_svc.utils ? embedded_svc.utils.getHostnameFromUrl(this.settings.iframeURL) : void 0;
      if (b && b.method && embedded_svc.isMessageFromSalesforceDomain(c) &&
        e && a.source === this.getESWFrame()) {
        if (c !== f)
          if ("session.frameReady" === b.method) this.settings.iframeURL = this.settings.iframeURL.replace(f, c);
          else return;
        a = b.method.split(".")[0].toLowerCase(); - 1 === this.availableFeatures.indexOf(a) ? (this.pendingMessages[a] || (this.pendingMessages[a] = []), this.pendingMessages[a].push({
          direction: "incoming",
          payload: b
  ...
  d.prototype.onButtonStatusChange = function () {
    ...
    a && (b = a.querySelector(".message")) && (this.isButtonDisabled ? (a.onclick = function () {}, a.classList.remove("helpButtonEnabled"), a.classList.add("helpButtonDisabled"),
      b.innerHTML = this.settings.disabledMinimizedText) : (a.onclick = this.onHelpButtonClick.bind(this), a.classList.remove("helpButtonDisabled"), a.classList.add("helpButtonEnabled"), b.innerHTML = this.settings.defaultMinimizedText))
  };
  ...
  d.prototype.setDefaultButtonText =
    function (a, b, c, e) {
  ...
  d.prototype.registerLinkAction = function (a, b) {
    ...
    this.validLinkActions[a] ||
      (this.validLinkActions[a] = []); - 1 === this.validLinkActions[a].indexOf(b) && this.validLinkActions[a].push(b);
  ...
  d.prototype.getLinkActionData = function () {
    window.location.search.replace(/([a-zA-Z0-9._]+)=([^&\s]+)/g, function (a, b, c) {
      a = b.toLowerCase();
      0 === a.indexOf("snapins.") && (a = a.replace("snapins.", ""), "action" === a ? (c = c.split("."), 2 === c.length && (this.settings.linkAction.feature = c[0], this.settings.linkAction.name = c[1])) : this.settings.linkActionParameters[a.toLowerCase()] =
        c)
    }.bind(this))
  };
